...
\begin{figure}[!h]
    ...
    \subfloat[][Classified as a "5"]{{\includegraphics[scale=0.4]{clean-5.png}}}
    \qquad
    \subfloat[][Classified as a "3"]
    {{\includegraphics[scale=0.4]{perturbed-5-classified-3.png}}} \caption{A
...
\Procedure{Targeted-PGD}{$\mathcal{C}_\theta, \vec{x}, t, \epsilon, p, m, \mu$}
    ...
    \For{$i \gets [1..m]$}
        \State $\vec{x_i} =$ \textproc{Targeted-FGSM}($\mathcal{C}_\theta, \vec{x_{i-1}}, t, \mu$)
...
\Procedure{Untargeted-PGD}{$\mathcal{C}_\theta, \vec{x}, y, \epsilon, p, m, \mu$}
    \State $\vec{x_0} \gets \vec{x}$
    \For{$i \gets [1..m]$}
        \State $\vec{x_i} =$ \textproc{Untargeted-FGSM}($\mathcal{C}_\theta, \vec{x_{i-1}}, y, \mu$)
        \State $\vec{x_i} \gets$ \textproc{Project}($\vec{x_i}, \mathcal{B}_\rho^p(\vec{x})$)
                \Comment{Project onto $l_p$ ball around $\vec{x}$}
    \EndFor
    \State \Return $\vec{x_m}$
...
\Procedure{Targeted-FGSM}{$\mathcal{C}_\theta, \vec{x}, t, \epsilon$}
    ...
    \State $\vec{x'}=\vec{x} - \epsilon \cdot \text{sign}(\vec{\Delta x})$
    \State \Return $\vec{x'}$
...
% \section*{Guarantees when we craft label noise}
% \begin{theorem}
%     \label{theorem:crafting-label-noise}
%     We are given a flip-budget $\eta$,i.e., given a dataset $S$, we are allowed
%     to change the labels of at max $\eta |S|$ points in the dataset. If there
%     exist constants $c_1, c_2, \rho$ and $\zeta^* \subset \mathbb{R}^d$, such
%     that:
%     \begin{enumerate}
%         \item $\zeta^* \in \underset{\zeta\subset\mathbb{R}^d, |\zeta| \leq \eta
%         |S|} {\arg\max}~\mathbb{P}_{\vec{x} \sim \mathcal{D}} \bigg
%         [~\underset{\vec{s} \in \zeta}{\bigcup}~\mathcal{B}_\rho^p(\vec{s})
%         \bigg]$
%         \item $\mathbb{P}_{\vec{x} \sim \mathcal{D}} \bigg [~\underset{\vec{s}
%         \in \zeta^*}{\bigcup}~\mathcal{B}_\rho^p(\vec{s}) \bigg ] \geq c_1$
%         \item $\forall \vec{s} \in \zeta,~~ \mathbb{P}_{\vec{x} \sim
%         \mathcal{D}} [\mathcal{B}_\rho^p(\vec{s})] \geq
%         \frac{c_{2}}{|\zeta|}$
%         \item $\forall \vec{s} \in \zeta,~\forall \vec{u}, \vec{v} \in
%         \mathcal{B}_\rho^p(\vec{s}),~c(\vec{u}) = c(\vec{v})$
%     \end{enumerate}
...
%     samples from $\mathcal{P}$, and labeling each with $c(.)$. This dataset $S$
%     is then label-flipped optimally, with the specified budget constraint of
%     $\eta$ to produce $S'$. If a classifier $\mathcal{C}$ is such that
%     \begin{enumerate}
%         \item $\mathcal{C}$ has a 100\% training accuracy on $S'$. This means it
%         gets the correct class on unflipped points, and has also memorized the
%         wrong label on flipped points. Further, on each training point, the
...
%     \begin{equation*}
%         \mathbb{P} \biggl [
%         \mathcal{R}_{\text{Adv}}^{2\rho-\frac{1}{\kappa}}
%         (\mathcal{C}) \geq
%         c_1 \biggr ]
%         \geq
%         1-\delta
%     \end{equation*}
%     if $n \geq \frac{|\zeta|}{c_2} \log \bigl( \frac{|\zeta|}{\delta} \bigr)$

% \end{theorem}
% \begin{proof}
% For $n$ samples, the probability that all the $l_p$ balls receive at least one
% training point inside of them, is:
% \begin{equation*}
% \begin{split}
%     \mathbb{P}[\forall \vec{s} \in \zeta, \exists (\vec{x}, y) \in S~\text{s.t.}~
%     \vec{x} \in \mathcal{B}_\rho^p(\vec{s})]
%     & = 1 - \mathbb{P} \biggl [~\underset{\vec{s} \in \zeta}{\bigvee} \nexists 
%     \vec{s'} \in \mathcal{B}_\rho^p(\vec{s}) \biggr ] \\
%     & = 1 - |\zeta| \biggl(1 - \frac{c_2}{|\zeta|}\biggr)^{n} ~\text{(Union bound)} \\
%     & \geq ~ 1 - |\zeta| e^{-n.\frac{c_2}{|\zeta|}} ~ \geq ~ 1 - \delta
% \end{split}
% \end{equation*}
% $n \geq \frac{|\zeta|}{c_2} \log \bigl( \frac{|\zeta|}{\delta} \bigr)$ samples
% suffice for this. Let $T$ be a dataset such that at least one point of $S$
% inside each $l_p$ ball in $\{\mathcal{B}_\rho^p(\vec{s}) \mid \vec{s} \in
% \zeta\}$ is label-flipped. Then, $S$ can be label-flipped to $S'$ such that:
% \begin{equation*}
%     \mathcal{R}_{\text{Adv}}^{2\rho-\frac{1}{\kappa}}
%         (\mathcal{C}^{S'}) \geq \mathcal{R}_{\text{Adv}}^{2\rho-\frac{1}{\kappa}}
%         (\mathcal{C}^{T}) \geq c_1
% \end{equation*}
...
\begin{figure}[!h]
    ...
    point]{{\includegraphics[scale=0.35]{thesis-learning-pockets-or-tweaking-boundary-1}}}
    \qquad
    \subfloat[][\centering Network teaking the decision boundary to
    "accommodate" a mislabaled
    point]{{\includegraphics[scale=0.35]{thesis-learning-pockets-or-tweaking-boundary-2.jpg}}}
